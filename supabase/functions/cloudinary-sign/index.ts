...
// The client uses signed uploads (no API secret on device) scoped to a per-node folder.
//
// Request body: { "node_id": "uuid", "kind": "story" | "photo" }
...
Deno.serve(async (req) => {
  ...
  if (!body.node_id || (body.kind !== "story" && body.kind !== "photo")) {
  ...
  // Verify membership in node before signing
  ...
  const folder = `node/${body.node_id}/${body.kind}s`;
